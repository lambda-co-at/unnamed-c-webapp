...
void hash_func(const char* value, char* dest, int algo, unsigned int flags) {
    ...
    if (gcry_md_test_algo(algo) == GPG_ERR_NO_ERROR) {
        ...
        strcpy(byte_result, gcry_md_read(Crypto_handle, algo)); /* only one algo in the obj, pass 0 for default */
         ...
         * and concatenate */
        for (int i = 0; i < stringlength(byte_result); i++) {
            sprintf(helper, "%02x", (unsigned char)byte_result[i]);
            stringconcat(dest, helper);
        }
